...
export async function scaffoldProject(deps: ScaffoldDeps, req: ScaffoldRequest): Promise<void> {
  ...
  // Every skill lives in agent-ops, dev or personal — dev projects go under
  // skills/shared/ like any other shared skill, matched via applies_to rather
  // than living in an app-specific folder or the target repo itself (§6).
  const skillFolder = req.type === "dev" ? `skills/shared/${req.name}` : `skills/personal/${req.name}`;
  ...
  entries.push({
    ...
    // type: "dev" has no skill pointer — the reusable workflow resolves which
    // shared skills apply by matching applies_to against repo/project_language.
    // type: "personal" has no repo/project_language to match against, so it
    // keeps an explicit pointer.
    ...(req.type === "personal" ? { skill_path: skillFolder } : {}),
  });
  await createOrUpdateFile(
    token,
    deps.controlRepoOwner,
