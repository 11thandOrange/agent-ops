...
3. Create one GitHub sub-issue per subtask via the sub-issues API (`POST /repos/{owner}/{repo}/issues/{issue_number}/sub_issues`) — the subtask's content is that sub-issue's **body**, not a line in a checklist. Use sub-sub-issues (up to 8 levels deep) only where a subtask is genuinely compound.
