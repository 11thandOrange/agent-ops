...
**(Re-revised) no Postgres, for now.** The original plan paired LiteLLM with Postgres for dynamically-issued, per-project virtual keys and DB-backed spend tracking/budget alerts. Decision: **drop Postgres for the initial deploy** — it only backs that optional layer, not the core alias-routing proxy above, which runs fine standalone. Trade-off accepted: there's no separately-scoped, revocable key distinct from `LITELLM_MASTER_KEY` right now — the master key is what GitHub Actions/the orchestrator actually use, so treat it with the same care a scoped key would get. Add Postgres back once there's real need for per-project dynamic keys or spend-by-repo breakdown (e.g. once `app-2`/`app-3` are running and cost visibility per repo actually matters).
...
- **(Re-revised) Hosting:** moved from the originally-provisioned Oracle Cloud Free Tier instance to **Google Cloud Run** — LiteLLM and the orchestrator (with the MCP server mounted on it) deploy there as separate services. Switched because the Oracle Cloud path required manual VM/VNIC/public-IP/Caddy setup with real friction, whereas Cloud Run gives HTTPS + a public endpoint automatically from a container push. Postgres is dropped for the initial deploy (see §3) rather than adding a Cloud SQL instance, to avoid a recurring cost until per-project dynamic keys are actually needed.
...
- **(Re-revised) Budget alert:** not LiteLLM's own DB-backed alerting (that needs the Postgres this doc just dropped, see §3) — GCP's native Billing → Budgets & alerts on the underlying Gemini/Vertex spend instead, set as soon as the gateway is live and billed (end of Phase 1), not deferred to a final hardening pass.
- **Quality gate:** self-hosted `PR-Agent` (`The-PR-Agent/pr-agent`) for review + self-hosted `qodo-cover` for test generation/coverage, both pointed at the LiteLLM gateway. Replaces the hosted Qodo product referenced earlier in this doc, with the fallback decision point in §4.4.
