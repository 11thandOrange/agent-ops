...
**(Revised)** a monthly budget alert on this gateway's spend is now set up immediately after the proxy is live (§8), not deferred to a final hardening pass — automated pipelines can run up costs faster than interactive chat, and by the time the original hardening phase would have caught it, Phases 3–9 would already have run real billed traffic.
...
- **Hosting:** Oracle Cloud Free Tier instance is provisioned — LiteLLM, the orchestrator, and the MCP server all run there.
...
- **(Revised) Budget alert:** set as soon as the LiteLLM gateway is live and billed (end of Phase 1), not deferred to a final hardening pass.
