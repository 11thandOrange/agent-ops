...
4. [x] Job wired through the orchestrator — but structurally different from the dev pipeline, not a re-use of it: `run_project_pipeline` now branches on call shape (`repo`+`issueNumber` → dev dispatch via GitHub Actions; `project`+`request` → `run_personal_pipeline.ts`, called directly by the orchestrator process itself, since personal projects have no repo/CI runner to dispatch to). That job loads the skill (fetched via GitHub Contents API — `skills/` isn't in the orchestrator's own Docker build context), gathers the posting via the selected sourcing method, calls the `planning` model alias through a new direct `integrations/litellm.ts` client (this didn't exist before — the dev pipeline only ever called the gateway from inside GitHub Actions, never from the orchestrator process), and renders `generated_pdf` documents via a new `integrations/pdf.ts` (pdfkit).
5. [ ] **Not yet run against real job postings or a real LinkedIn session.** Everything above is implemented and typechecks, but: no live LiteLLM gateway credentials, no `LINKEDIN_STORAGE_STATE_PATH` (authenticated Playwright session), and no `JOB_API_BASE_URL`/`JOB_API_KEY` were available in the environment this was built in. In particular, `sourcing/scraping.ts`'s selectors (`.jobs-description__content`, `.jobs-box__html-content`) are a first draft against LinkedIn's presumed page structure, not verified against a live page — check these against a real posting before trusting the scraping path. Playwright also needs browser binaries available in the deployed Cloud Run image (the base `node:20-slim` image in `orchestrator/Dockerfile` doesn't have them) — either switch to a Playwright-provided base image for the final stage or add an explicit browser-install step before this path can run in production.
6. [ ] Test with a handful of real job postings, end to end, confirming the human-submit handoff is clear and nothing auto-submits.
7. [x] **(Revised)** notifications: the result comes back through whichever chat platform is connected to the MCP server. There is no separate notification channel to add or configure — Bird is not part of this system.

**Checkpoint — not yet met:** the code path exists and typechecks (`npm run typecheck`/`npm run build` pass), but a real job application's package hasn't been generated end to end against live credentials yet. Remaining before this checkpoint is genuinely met: wire real `LITELLM_PROXY_URL`/`LITELLM_VIRTUAL_KEY`, a real `LINKEDIN_STORAGE_STATE_PATH` session, confirm the scraping selectors against an actual posting (or switch the default to `manual`/`api` if they don't hold up), and fix the Playwright-in-Cloud-Run browser-binary gap in step 5.
...
- `run_project_pipeline` dispatch itself is two code paths, not one function branching on a type flag — dev and personal request shapes are structurally distinct (`chat_command.ts`), so a personal-shaped call can't accidentally hit the dev dispatcher or vice versa.
